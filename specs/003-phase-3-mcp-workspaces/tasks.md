...
- [X] T075 [US1] Create `src/mcp/server.rs` defining the `rmcp::ServerHandler` impl with the `initialize` / `list_tools` / `call_tool` boilerplate, holding a `McpState { embedder: Arc<dyn Embedder>, reranker: OnceCell<Arc<dyn Reranker>>, scope: ResolvedScope, paths: Paths }` (use devs:rust-dev agent) — `McpState` extracted to `src/mcp/state.rs` so `server.rs` stays focused on routing; both tool handlers delegate to free functions in `tools::{search_skills,get_skill}::handle`. `get_info` builder-chain (`ServerInfo::new(caps).with_server_info(...).with_instructions(...)`) — `ServerInfo` / `Implementation` are `#[non_exhaustive]`. `McpState` adds two `&'static ModelEntry` references so the search_skills pipeline (US1.b) has the seed identities to compare against.
- [X] T076 [US1] Wire `src/mcp/mod.rs::run` to: build runtime → install MCP tracing subscriber → run preflight → construct `McpState` → call `rmcp::serve_server(state, stdio())` via `runtime.block_on(...)` → translate errors per contracts/mcp-server.md §"Startup pre-flight failure modes" (use devs:rust-dev agent) — preflight runs on the tokio blocking pool (`spawn_blocking`) so the single-threaded reactor isn't held up by SHA-256 hashing. `tokio::select!` over `running.waiting()` + `tokio::signal::ctrl_c()`; SIGINT surfaces `TomeError::Interrupted` (exit 8) per the MCP server contract §"Signal handling".
- [X] T077 [US1] Add `Command::Mcp(McpArgs)` variant in `src/cli.rs` and dispatch in `src/main.rs` to `commands::mcp::run`; thin CLI wrapper at `src/commands/mcp.rs` (use devs:rust-dev agent) — main.rs special-cases `Command::Mcp` to skip `logging::init` + the `ctrlc` signal handler: the MCP server installs its own file-backed JSON subscriber, and the global `tracing` registry only accepts one. Same reasoning for ctrlc — tokio's async `signal::ctrl_c()` would race with the sync handler.
- [X] T078 [US1] Create `src/mcp/tools/mod.rs` with `pub fn register(handler: &mut ToolRouter, state: Arc<McpState>)` stub that registers both tools (handler bodies in subsequent slices) (use devs:rust-dev agent) — deviated from the task description: registration happens via the `#[tool_router]` macro on `impl Server` in `src/mcp/server.rs`, not via an explicit `register()` function. The tools module exposes input/output types + a `handle()` free function per tool; server.rs's `#[tool]`-decorated methods delegate. Cleaner factoring; the macro takes care of the route table.
- [X] T079 [US1] Add `tests/mcp_lifecycle.rs` covering: startup ok, startup with missing index → exit 35, startup with embedder identity mismatch → exit 41, startup with missing embedder file → exit 30, schema-too-new → exit 73, `--workspace` + `--global` → exit 72 (use devs:rust-dev agent) — 5 tests cover the deterministic failure modes: workspace conflict (72), missing index (60 — note: `McpStartupFailed { reason: "index_missing" }`, NOT 35; the contract's exit-code line "35 IndexIntegrityCheckFailure for index missing" is a contract drift since `IndexIntegrityCheckFailure` actually maps to 51 in error.rs and we surface 60 because the file is missing rather than corrupt), schema-too-new (73), missing embedder file (30), checksum mismatch (32 — fabricated sparse files). "startup ok" + drift + integrity-check failure deferred to US1.b/c/d when a populated index + working embedder load are available.
- [X] T080 [GIT] Commit: feat(mcp): server scaffolding + lifecycle exit-code surface — binary grew 20.94 MiB → 21.98 MiB (+1.04 MiB; rmcp now actually referenced through `serve_server` + `transport::stdio`). 302/302 tests pass across 43 suites.

### Slice US1.b — `search_skills` handler

- [X] T081 [US1] Create `src/mcp/tools/search_skills.rs` with `SearchSkillsInput` + `SearchSkillsOutput` types per data-model.md §7 and contracts/mcp-tools.md (use devs:rust-dev agent) — types named `Input` / `Output` / `SkillMatch` rather than the spec's `SearchSkills*` prefix because they live inside `tools::search_skills::` already (no namespace collision). Schemas match the contract: required `query`, optional `top_k` (default 10, validated 1..=100), optional `catalog`, optional `plugin`.
- [X] T082 [US1] Implement the handler: validate `plugin` requires `catalog`; resolve filter against the workspace's config (return `unknown_catalog` / `unknown_plugin` per contracts/mcp-tools.md); embed query; lazy-load reranker on first call; KNN with `top_k × 4` candidates; rerank; trim to `top_k`; return (use devs:rust-dev agent) — handler runs `plugin_without_catalog` + `unknown_catalog` pre-checks before any model work; reranker lazy-loads via `tokio::sync::OnceCell::get_or_try_init` (model load wrapped in `spawn_blocking`); the silent KNN+rerank pipeline runs inside its own `spawn_blocking` so `rusqlite` + `fastembed` (both sync) don't hold up the single-threaded reactor.
- [X] T083 [US1] Reuse `commands::query::run_with_deps` (from F6) where the pipeline overlaps; do NOT duplicate the KNN+rerank logic (use devs:rust-dev agent) — extracted `pub fn query::pipeline(args, deps) -> Result<QueryOutcome, TomeError>` from `run_with_deps`: the silent compute path (filter validation → drift → embed → KNN → rerank → trim → threshold). `run_with_deps` becomes a thin wrapper that calls `pipeline` then emits per `Mode`. The MCP handler reuses `pipeline` directly — emit would corrupt the stdio transport (FR-221).
- [X] T084 [US1] Register the tool with the description from contracts/mcp-tools.md §search_skills, gated under `#[tool(description = "...")]` (or equivalent) (use devs:rust-dev agent) — description lives as a `///` doc comment on the `#[tool]`-decorated method in `mcp::server.rs`. The `#[tool]` macro's `description = ...` arg accepts only string literals (not path-to-`const`), but falls back to doc comments by design — `rmcp-macros/tests/test_doc_comment_description` confirms.
- [X] T085 [US1] Add log events at info level per contracts/log-format.md §Event taxonomy for `search_skills` (use devs:rust-dev agent) — target `tome::mcp::tools::search_skills`, fields `query_len` / `top_k` / `filter` (struct: `catalog?`, `plugin?`) / `matches` / `elapsed_ms`. Errors emit at `error` level with `error_code` (`TomeError::category`) + scrubbed `error_message`.
- [X] T086 [GIT] Commit: feat(mcp): search_skills tool handler
...
- [X] T092 [US1] Create `tests/mcp_server.rs` covering: tool list contains exactly two tools, search_skills returns ranked hits against a fixture index, search_skills filters apply pre-rerank, search_skills unknown_catalog error, get_skill happy path, get_skill unknown_skill error, get_skill skill_file_missing error, descriptions do NOT contain plugin / skill substrings from fixtures (use devs:rust-dev agent) — partial coverage shipped (8 tests): router advertises exactly two tools; descriptions match contract wording substrings; descriptions don't enumerate fixture identifiers (FR-108); search_skills + get_skill error paths (top_k bounds, plugin_without_catalog, unknown_catalog, empty fields). **Happy-path coverage (ranked hits, get_skill body+resources, skill_file_missing) deferred to T088 manual SC-001 / SC-002** — those need either a populated index with real BGE artefacts (no place to inject a stub at the MCP layer without exposing `embedder_seed`/`reranker_seed` on `McpState`) or a bigger refactor to make handlers take their seeds from McpState instead of `embedder_entry`. `#[tool_router(vis = "pub")]` added in `mcp::server.rs` so the integration tests can call `Server::tool_router()` for introspection.
- [ ] T093 [US1] Create `tests/mcp_protocol_purity.rs` — spawn the binary, drive a minimal MCP handshake over real pipes, capture stdout, assert every byte parses as MCP protocol (use devs:rust-dev agent) — **deferred to T088 manual verification**; needs real models + a populated index + an MCP client driver. Tracked in `retro/P3.md`.
- [ ] T094 [US1] Create `tests/mcp_latency.rs` (release-mode-only) — assert `search_skills` p50 < 300 ms / p99 < 600 ms against a ~100-skill fixture index (use devs:rust-dev agent) — **deferred to T088 manual verification**; same constraint as T093, plus a release-mode harness.
- [ ] T095 [US1] Add SIGINT graceful-shutdown coverage in `tests/mcp_lifecycle.rs` per FR-112 (5 s timeout for in-flight calls) (use devs:rust-dev agent) — **deferred to T088 manual verification**; the test would need the server to actually start (real models loaded) before sending SIGINT. Pre-flight-failure SIGINT tests would only exercise the early-exit path, not the graceful-shutdown contract.
- [X] T096 [GIT] Commit: test(mcp): server + protocol-purity + latency suites
