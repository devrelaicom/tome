...
| 14 | `WorkspaceAlreadyExists { name }` | `tome workspace init <name>` against an existing name | No |
...
| 20 | `SummariserFailure { kind }` | Summariser model missing at use time; model checksum mismatch at use time; `LlamaBackend::init()` failure; model output empty or unparsable | Yes for `ModelMissing` (re-download); No for others |
