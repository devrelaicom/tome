...
- **Current phase:** **Phase 7 planning complete (spec + plan)** — *Beta hardening + public release*. `/sdd:specify` + `/sdd:plan` done on `007-phase-7-beta-release` (no PRD; the two beta-readiness audits are the source of WHAT); implementation not started, `/sdd:tasks` pending. **Phase 6 shipped (v0.6.0)** — hooks + agents; the v0.6.0 git tag + `cargo publish` + release-notes remain reserved for user push (and are now the subject of the Phase 7 release wrapper). Phase 5 (v0.5.0) 2026-05-27; Phase 4 (v0.4.0) 2026-05-26.
