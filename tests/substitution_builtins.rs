...
/// Process-global serialisation for tests in this file that install one
/// of the substitution override slots (`SUBSTITUTION_CLOCK_OVERRIDE`,
/// `PLUGIN_DATA_DIR_OVERRIDE`, `WORKSPACE_DATA_DIR_OVERRIDE`). The
/// `OnceLock<Mutex<_>>` slots in the substitution module are
/// process-global, so two cargo-parallel tests installing different
/// values race; this mutex serialises every test in the binary that
/// touches an override.
///
/// Mirrors the `OVERRIDE_MUTEX` pattern from Phase 4 / US3.c-1's
/// `tests/harness_sync_stub.rs`.
...
fn create_dir_all_failure_surfaces_plugin_data_dir_creation_failed() {
    ...
    let ctx = SubstitutionContext::builder()
        ...
        .plugin_data_dir(PathBuf::from("/x"))
        .workspace_name("global")
        .workspace_data_dir(PathBuf::from("/x"))
        .clock(OffsetDateTime::UNIX_EPOCH)
        .paths(paths)
        .build()
        .unwrap();
