...
/// Serialise on the shared [`crate::common::SUBSTITUTION_OVERRIDE_MUTEX`] so
/// every `substitution_*` test in this consolidated binary that touches a
/// process-global override slot is mutually exclusive (the former per-file
/// mutex only covered a single process).
