//! US3.3 byte-stable registration + manifest pins for `devin`.
//!
//! Drives the REAL `sync_project` over the `devin` module with ONE enabled
...
//!    harness-NATIVE event key (`PreToolUse`) at the document root (no wrapper); and
...
const HOOK_ARRAY: &str = r##"[
  {
    ...
    "hooks": [
      {
        "type": "command",
        "command": "tome harness run-hook --event PreToolUse --harness devin --workspace test-workspace"
      }
    ]
  }
]"##;
...
#[test]
fn devin_run_hook_registration_and_manifest_pins() {
    ...
    assert_eq!(
        ...
        HOOK_ARRAY,
        "run-hook entry bytes drifted for devin",
    );
    ...
    let manifest_path = fx.paths.hooks_manifest(&fx.workspace, "devin");
    ...
    assert_eq!(got_rest, MANIFEST, "manifest bytes drifted for devin");
}
