//! US3.3 byte-stable registration + manifest pins for `gemini`.
//!
//! Drives the REAL `sync_project` over the `gemini` module with ONE enabled
...
//!    harness-NATIVE event key (`BeforeTool`) under the `hooks` container; and
...
const HOOK_ARRAY: &str = r##"[
  {
    "hooks": [
      {
        "name": "tome-hook-dispatch",
        "type": "command",
        "command": "tome harness run-hook --event PreToolUse --harness gemini --workspace test-workspace"
      }
    ]
  }
]"##;
...
#[test]
fn gemini_run_hook_registration_and_manifest_pins() {
    ...
    let hook_path = fx.project.join(".gemini/settings.json");
    ...
    // The native file-key is `BeforeTool` but the command names the CC event.
    crate::common::canonicalize_tome_hook_command_leaves(
        &mut doc,
        &["harness run-hook --event PreToolUse --harness gemini --workspace test-workspace"],
    );
    let arr = &doc["hooks"]["BeforeTool"];
    assert_eq!(
    ...
    let manifest_path = fx.paths.hooks_manifest(&fx.workspace, "gemini");
    ...
    assert_eq!(got_rest, MANIFEST, "manifest bytes drifted for gemini");
}
