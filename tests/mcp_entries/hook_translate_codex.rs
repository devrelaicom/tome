//! US3.3 byte-stable registration + manifest pins for `codex`.
//!
//! Drives the REAL `sync_project` over the `codex` module with ONE enabled
...
const HOOK_ARRAY: &str = r##"[
  {
    ...
    "hooks": [
      {
        "type": "command",
        "command": "tome harness run-hook --event PreToolUse --harness codex --workspace test-workspace"
      }
    ]
  }
]"##;
...
#[test]
fn codex_run_hook_registration_and_manifest_pins() {
    ...
    assert_eq!(
        ...
        HOOK_ARRAY,
        "run-hook entry bytes drifted for codex",
    );
