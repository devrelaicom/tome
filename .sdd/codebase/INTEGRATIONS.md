...
> **Last Updated**: 2026-05-27 (Phase 5 complete — v0.5.0 shipped; per-entry invocability matrix end-to-end; helper consolidation landed: `build_context_for_entry` SSOT + `validate_db_stored_path` SSOT; description truncation pattern propagated; 1193 tests across 151 suites + 1 new gap-2 test)
...
**Context building** (`src/substitution/context.rs` — `SubstitutionContextBuilder`, Phase 5 Polish: shared via `build_context_for_entry`):
...
## Path Traversal Defence (Phase 5 Polish)
...
- Single validation source prevents duplicated logic; aligns with defence-in-depth principle
