...
> **Last Updated**: 2026-05-27 (Phase 5 / US5 complete; per-entry invocability flags + doctor extensions; 0 security findings from reviewer pass; 4 blockers fixed across Phase 5)
...
## Deferred Findings from Phase 5 Review (PR #110–#114, US1 + US2 + US3 + US4 + US5)

Phase 5 / US1+US2+US3+US4+US5 security audits produced fixes applied in PR #110–#114+ and subsequent:
...
| **C-1-US5** | **Medium Security** | **Symlink path disclosure in doctor output (ACCEPTED RISK)** | ✅ **Documented in PR #124–#125 / US5.c (SEC-019-US5-P5)**; v0.6+ cap-std hardening planned |
...
| (deferred) | Security | Prompt-injection trust boundary documentation (Phase 6+ governance doc) | Phase 6+ documentation |
...
| `time` | 0.3.x (Phase 5 US2) | Clock + timestamp formatting for substitution built-ins | Monitor for format-string safety; used for deterministic test seams (`SUBSTITUTION_CLOCK_OVERRIDE`) | Stable |
...
## v0.6+ Polish Backlog (Post-Phase 5)

Phase 5 / US5 audit and closure deferred items to v0.6+ for investigation + implementation:

| # | Category | Item | Reason | Target | Owner |
|---|----------|------|--------|--------|-------|
| R-M2 | Rust | Orphaned-workspace-dir floods orphan report | Doctor reports ALL `.tome.tmp.*` dirs regardless of parent; contract clarification needed on what constitutes reportable orphan | Phase 5 Polish / v0.6 | — |
| 6x | Rust | Cosmetic nits across US5 code | Per `review/us5-disposition.md` (R-M3/R-M4/M-1 + 3 more) | Phase 5 Polish v0.5.1+ | — |
| C-1 | Security | TOCTOU symlink path disclosure in doctor (accepted risk) | Cap-std / O_NOFOLLOW hardening to prevent openat(2) TOCTOU between symlink-check and access. Currently defended by "catalog trusted-on-enrol"; symlink-skip on walk, refusal on write | Phase 6+ via cap-std | — |
| S-M1 | Security | Terminal escape-sequence sanitization for plugin/entry names | Parse-time sanitization or output-time scrubbing of ANSI sequences in plugin/entry/catalog names before display | Phase 6+ (low priority) | — |
| TOCTOU | Security | Residual TOCTOU in `walk_resources` (US4.d disposition) | Between `file_type()` lstat + `read_dir()` on resource subdirs; accept as designed (catalog trusted-on-enrol) OR upgrade to cap-std openat/O_NOFOLLOW | Phase 6+ cap-std spike | — |
| Refactor | Architecture | Read-only DB open refactor (from Phase 3 backlog) | Consolidate ~9 `SELECT` sites that currently open `open_read_only` before expensive compute; harden to single canonical read path | Phase 6+ | — |
...
*Last refreshed 2026-05-27 against Phase 5 / US5 complete source (1193 tests passing, ~150 suites); 0 security findings from Phase 5 / US5 reviewer pass; 4 critical blockers fixed across Phase 5.*
