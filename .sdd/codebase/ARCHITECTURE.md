...
- **Design**:
  ...
  - Tests inject synthetic migrations via `thread_local! { MIGRATIONS_OVERRIDE }` to verify the framework end-to-end without waiting for real schema changes. Phase 3 US5 test-only: `tests/schema_migration_e2e.rs` exercises the migration engine directly and the atomicity tests use helper `write_index_db_with_schema_version` to craft specific index states (Phase 3 US5).
...
- **Migrations** (Foundational F7, Phase 3 US5 test-only):
  ...
  - Phase 2 ships zero registered migrations (plan defers schema changes to Phase 10); tests use `MIGRATIONS_OVERRIDE` to verify the framework end-to-end.
  - Phase 3 US5 adds `tests/schema_migration_e2e.rs` for integration test coverage and extends `tests/atomicity.rs` with interrupt-injection on migration paths.
- **Key Operations**:
