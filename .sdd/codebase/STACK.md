...
Phase 1–4 is a CLI application. Phase 3 Foundational F8 introduces MCP server scaffolding scoped to `src/mcp/`, wired in Phase 3 / US1. Phase 4 US1 adds harness integration orchestration. Phase 4 US2 extends workspace lifecycle with list/rename/sync/regen-summary/remove.
...
|---------|---------|---------|-------------|
| `serde` + `serde_derive` | 1.x | Configuration and manifest (de)serialisation | All TOML parsing for `config.toml` and `tome-catalog.toml`; Tome-owned structs use `#[serde(deny_unknown_fields)]` (FR-013a boundary); Phase 4 adds multi-level settings composition (project/workspace/global) with strict parsing; emit-only types (`WorkspaceInfo`, `InitOutcome`, `BindOutcome`, `RegenSummaryOutcome`, `SyncOutcome`) carry `Serialize` only |
| `toml` | 0.8 | TOML format support | Tome-owned manifest and config file parsing; workspace init and settings file generation; Phase 4 US2: workspace settings composition |
| `serde_json` | 1.x (with `preserve_order`) | JSON serialisation with preserved key order | `--json` mode formatting for stdout; `ModelManifest` serialisation; `BindOutcome` serialisation; `--json` byte-stability tests pin wire format; Phase 4 US1: preserves key order in harness MCP config output; Phase 4 US2: workspace list/rename/sync/regen-summary outcome serialisation |
| `toml_edit` | 0.25 | Comment/order-preserving TOML editor | Phase 4 F1–F11 + US1–US2: `src/settings/`, `src/harness/rules_file.rs`, `src/harness/mcp_config.rs` — read-modify-write harness MCP config files and workspace settings without losing comments/formatting; Phase 4 US2: `workspace::rename` rewrites workspace settings files, `workspace::regen_summary` updates `[summaries]` table |
...
| `tracing` + `tracing-subscriber` | 0.1, 0.3 | Structured logging to stderr | Diagnostic output orthogonal to `--json` stdout; Phase 3 F8 enables `json` feature for MCP log subscriber (JSON-lines to file via `src/mcp/log.rs`); custom `ContractEventFormat` renders contract-pinned field names; Phase 4 US2: unchanged |
| `sha2` | 0.10 | Content-addressed cache naming and model integrity | URL hashing for `cache_dir_for()` in `src/paths.rs`; model download verification; workspace registry deduplication; Phase 4 US2: workspace rename uses hash for refcount queries |
| `regex` | 1.x | Credential scrubbing patterns | Git stderr sanitisation in `src/catalog/git.rs`; model download URLs; MCP log field scrubbing; Phase 4 US1: harness rules-file marker line detection; Phase 4 US2: unchanged |
...
| `time` | 0.3 | Timestamp formatting and parsing | Logging and manifest timestamps; RFC 3339 serialisation; Phase 4 F1–F11 + US1–US2: `CachedSummaries.generated_at` round-trip TOML datetime literals ↔ RFC 3339 strings |
| `rusqlite` | 0.32 (`bundled`) | Embedded SQLite (statically linked, no system dep) | `src/index/*` — the local skill index database; WAL mode + advisory lockfile (FR-040); workspace-scoped index in Phase 3 Foundational F1; Phase 4 F11: `workspace_catalogs` junction table (sole source of truth); Phase 4 US1: `workspace_projects` table for 1:1 binding (primary key on `project_path`); Phase 4 US2: workspace queries (list, rename, remove) read/update workspace_projects + workspace_catalogs junctions |
...
| `serde_yaml` | 0.9 | Lenient YAML frontmatter parsing for third-party `SKILL.md` | `src/plugin/frontmatter.rs` — parses upstream metadata leniently (FR-013a boundary); Phase 4 US1: `.tome/RULES.md` frontmatter parsed via same lenient parser; Phase 4 US2: unchanged |
...
| `comfy-table` | 7.x | Table rendering for human-mode list/show output | `src/presentation/tables.rs` — `tome plugin list`, `tome models list`, query results, doctor reports, `tome harness list`, Phase 4 US2: `tome workspace list` human output |
...
| `llama-cpp-2` | 0.1 | Rust bindings to `llama.cpp` | Phase 4 F1–F11 + US1–US2: `src/summarise/` — LLM inference for workspace summaries (Qwen2.5-0.5B-Instruct GGUF); sync API, process-wide `LlamaBackend` singleton via `std::sync::OnceLock`; F6 ships skeleton (`StubSummariser`), US2 exercises via `workspace regen-summary` (real summariser pending US4.a), US4.a will wire full summarisation lifecycle |
...
| **Binary Size** | < 50 MB stripped on release builds (enforced by CI); Phase 4 / F1–F11 projection: ~28.4 MiB on macOS arm64, ~34 MB on Linux x86_64; Phase 4 US1–US2 adds workspace lifecycle + summariser skeleton but no new inference overhead (llama model not activated until US4) |
...
| **Configuration** | Global: `${XDG_CONFIG_HOME}/tome/settings.toml` (Phase 4 F8+); Workspace: `${WORKSPACE}/.tome/settings.toml`; Project: `${PROJECT}/.tome/config.toml` (Phase 4 US1); Phase 4 F1–F11 + US2: settings composition framework via `src/settings/` with multi-level resolver + layer precedence (project > workspace > global); Phase 4 US2: workspace `rename` uses composition resolver to update marker identities |
| **Summariser caching** | Phase 4 US2: per-workspace `[summaries]` table in `${WORKSPACE}/.tome/settings.toml` with `short_summary`, `long_summary`, `generated_at` (RFC 3339 datetime literal); regenerated via `tome workspace regen-summary` or automatic triggers (pending US4) |
...
| **Sync enforcement** | Pre-commit hook runs `cargo test` with sync-boundary test; CI gates all PRs on boundary enforcement; Phase 4 US2: workspace commands stay sync-only |
...
- Phase 4 US2: no new feature flags required; workspace lifecycle uses existing composition framework
...
*This document captures what executes in Tome at Phase 4 Foundational F1–F11 + US1–US2 complete. Phase 4 adds `llama-cpp-2`, `toml_edit`, and `serde_json/preserve_order`; harness module abstraction with five concrete impls; project binding infrastructure; multi-level settings composition framework; workspace lifecycle (list/rename/sync/regen-summary/remove) with atomic marker relocation. Binary size projection remains ~28–34 MB, well under the 50 MB cap.*
