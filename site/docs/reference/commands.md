...
| `disable <catalog>/<plugin>...` | `--catalog`, `--force`, `--sync` | Disable one or more plugins; embeddings stay on disk so re-enable is cheap. Accepts multiple ids and `*` wildcards (e.g. `disable midnight/*`) plus `--catalog <name>` for bare or wildcard names, mirroring `enable`. A zero-match wildcard is an error; a batch keeps going past a bad id and surfaces the first failure's exit code. `--force` skips the confirmation prompt (a batch prompts once, naming every plugin). `--sync` applies the change to your harnesses inline (runs the same propagation as `tome sync` over every bound project); without it, disable prints a reminder to run `tome sync`. |
