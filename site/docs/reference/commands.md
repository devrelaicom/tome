...
| `info [<name>]` | `--details` | Report a workspace's details. Read-only; never acquires the advisory lock. Defaults to the resolved workspace. `--details` expands the enabled-plugins section into a per-entry routing-tier breakdown. |
...
Configure target coding agents across the seventeen supported harnesses
(Claude Code, Codex, Cursor, Gemini CLI, OpenCode, GitHub Copilot, Zed, Devin,
Kiro, and more). Bare `tome harness` enumerates every supported harness.
