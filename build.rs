...
/// Vendored model-registry snapshot must stay well under the 50 MB binary cap.
const MODEL_REGISTRY_BYTE_BUDGET: u64 = 2 * 1024 * 1024; // 2 MiB

fn main() {
    compile_sqlite_vec();
    generate_meta_skill_manifest();
    generate_harness_plugin_manifest();
    generate_model_registry_manifest();
}
...
// ---------------------------------------------------------------------------
// Embedded model-registry snapshot (Phase 13 Task 2)
// ---------------------------------------------------------------------------
//
// The vendored `assets/model-registry/registry.json` (trimmed snapshot shape)
// is embedded into the binary at build time. At runtime, `ModelRegistry::load`
// checks for a fresher override at `~/.tome/cache/model-registry.json`; if
// absent or corrupt it falls back to this baked snapshot. The CI refresh job
// (Task 7) keeps the vendored file current; this build gate ensures it is
// structurally valid at every build.

fn generate_model_registry_manifest() {
    let manifest_dir =
        PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR"));
    let asset = manifest_dir.join("assets/model-registry/registry.json");
    println!("cargo:rerun-if-changed={}", asset.display());

    let bytes = fs::read(&asset).unwrap_or_else(|e| panic!("read {}: {e}", asset.display()));
    assert!(
        (bytes.len() as u64) <= MODEL_REGISTRY_BYTE_BUDGET,
    ...
    let out_dir = PathBuf::from(env::var_os("OUT_DIR").expect("OUT_DIR"));
    let generated = out_dir.join("model_registry_snapshot.rs");
    let src = format!(
        "// @generated by build.rs — do not edit. Source: assets/model-registry/registry.json\n\
         pub static MODEL_REGISTRY_SNAPSHOT: &[u8] = include_bytes!({});\n",
        rust_str(&asset.to_string_lossy())
    );
    fs::write(&generated, src).unwrap_or_else(|e| panic!("write {}: {e}", generated.display()));
}
